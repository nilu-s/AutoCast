...
function applySegmentPadding(allSegments, totalDurationSec, beforeMs, afterMs, options) {
    ...
    for (var t = 0; t < allSegments.length; t++) {
        ...
        for (var i = 0; i < segs.length; i++) {
            ...
            var padded = {};
            for (var key in s) {
                if (s.hasOwnProperty(key)) padded[key] = s[key];
            }
            padded.start = paddedStart;
            padded.end = paddedEnd;
        ...
        for (var k = 0; k < merged.length; k++) {
            merged[k].durationMs = Math.round((merged[k].end - merged[k].start) * 1000);
        }
        out.push(merged);
    }
    return out;
...
module.exports = {
    applySegmentPadding: applySegmentPadding,
    findNextOtherTrackStart: findNextOtherTrackStart,
    findOtherTrackEndNearStart: findOtherTrackEndNearStart
};

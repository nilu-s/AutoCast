 ...
 * Usage: node test/run_all_tests.js
 */
...
global.assertApprox = function (actual, expected, tolerance, message) {
    ...
    if (Math.abs(actual - expected) > tolerance) {
        throw new Error((message || 'Value mismatch') + ': expected ' + expected + ' ± ' + tolerance + ', got ' + actual);
    }
};
...
console.log('║   Results: ' + totalPassed + '/' + totalTests + ' passed' +
    ...
    '              ║'.substring(0, 23 - String(totalPassed).length - String(totalTests).length));
console.log('╚════════════════════════════════════╝');
